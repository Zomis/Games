...
const supportedGames = {
  ...
  Hanabi: {
    ...
    viewTypes: {
      card: {
        component: HanabiCard,
        binds: (v) => ({ card: v, doubleView: true }),
      },
    },
    component: Hanabi,
  },
  ...
  "Dungeon Mayhem": {
    ...
    actions: {
      ...
      target: (target) =>
        ...
        target.shieldCard +
        ";discarded-" +
        target.discardedCard,
    },
    viewTypes: {
      card: {
        component: DungeonMayhemCard,
        binds: (v) => ({ card: v, actions: { available: {} } }),
      },
    },
...
function enabledGames() {
  return Object.keys(supportedGames).filter(
    (key) => supportedGames[key].enabled !== false
  );
}
...
export default {
  ...
  resolveActionKey(supportedGame, actionData, actionChoice) {
    ...
    if (!actionKeys) {
      console.log(
        "No actionKeys set for",
        actionName,
        "default to same as serialized value",
        value
      );
      return value;
    }
